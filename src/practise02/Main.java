...
public class Main {
	...
	public static void main(String[] args) {
		...
		// A <- B <- C の継承関係があるとき、
		// [A a = new C();]  OK
		// → C は A のサブクラス（C is-a A）
		// → C は A の全メソッド・フィールドを必ず持っているため、A として扱っても安全
		// [C c = new A();]  NG
		// → A は C の持つメソッド・フィールドを持っていない
		// → C として扱うと存在しないメソッドを呼べてしまうため型安全性が壊れる
		// ↑ の基礎が、ワイルドカードのextendsやsuperの挙動となる。
		//
		// X<? extends B>（上限境界）
		// → 入るのは B または C（B のサブタイプ）
		// → 実際の型が B か C かは不明だが、上限が B で確定しているため「B として安全に取り出せる」
		// → ただし実際の型が不明なので「安全にセットできるのは null のみ」（読み取り専用）
		//
		// X<? super B>（下限境界）
		// → 入るのは Object / A / B（B のスーパタイプ）
		// → 下限が B で確定しているため「B を安全にセットできる」
		// → ただし実際の型が Object / A / B のどれか不明なので「取り出すときは Object としてのみ安全」
